...
print(alpha)
pool_size_upper = int(sys.argv[2])
additional_instance_per_iter = int(sys.argv[3])
print(additional_instance_per_iter)
...
for siter in range(sample_iters):
    ...
    pdt2= sess.run(y, feed_dict={x_data: test_feature[index1][:,:,:fealen]})
    ...
    for i in range(0, len(index1)):
        if pdt2[i] == 0:
            ...
            H_x   = H2
            H_x_n = H1
        if H_x[:,index_test[index1[i]]].min()==0:
            ts[i] = 2
        elif len(H_x_n)==0 or len(H_x)==0:
            ts[i] = 0
        else:
            ts[i] = H_x_n[:,index_test[index1[i]]].min() / H_x[:,index_test[index1[i]]].min()
    ...
    H_train = np.concatenate((H_train, H_test[indices]), axis=0)
