...
infile.read("config/cfg2gmm.ini")
modelpath="models/iccad2gmm/"
datapath="benchmarks/fts/data2"
fealen=32
...
additional_instance_per_iter = 60
additional_instance_num = min(test_data.maxlen, additional_instance_num)
sample_iters = (additional_instance_num-start_to_active)//pool_size_upper + 1
...
H_set = np.zeros((len(test_data.ft_buffer), len(test_data.ft_buffer)))
#print("Compute H_set:  0%")
for i in range(0, len(test_data.ft_buffer)):
    for j in range(0, len(test_data.ft_buffer)):
        H_set[i][j] = np.linalg.norm(test_data.ft_buffer[i]-test_data.ft_buffer[j])
    print("\r","Compute H_set:  "+str(i * 100//len(test_data.ft_buffer) + 1)+"%",end="",flush=True)
...
for siter in range(sample_iters):
	...
	H_loop_n = H_loop[idx_n2]
	H_loop_p = H_loop[idx_p2]
	...
	for i in range(0, len(test_feature)):
	    if pdt[i] == 0:
	        ...
	        H_x   = H2
	        H_x_n = H1
	    ts[i] = (H_x_n.sum(axis=0)[i]/len(H_x_n)) / (H_x.sum(axis=0)[i]/len(H_x))
